...
│                      Your Application                            │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │   MGraph-AI Cache Client (Python Library)                  │ │
│  │   - Service__Fast_API__Client                              │ │
│  │   - Type-Safe validation                                   │ │
│  │   - Three execution modes                                  │ │
│  └───────────────────┬────────────────────────────────────────┘ │
└────────────────────────┼──────────────────────────────────────────┘
                         │
                         │ HTTP/HTTPS (or in-process)
                         │
        ┌────────────────┴────────────────┐
        │                                 │
        ...
        │                                 │
┌───────▼──────────────┐          ┌──────▼──────────────┐
│  Cache Service       │          │  FastAPI TestClient │
│  (FastAPI Backend)   │          │  (Same Process)     │
│                      │          │                     │
│  - REST API          │          │  - No Network       │
│  - Strategy Handler  │          │  - Instant Response │
│  - Storage Abstraction│         │  - Memory Storage   │
└──────────┬───────────┘          └─────────────────────┘
           │
           │ Memory-FS Layer
           │
...
```
Your App → Python Client → HTTP/HTTPS → Cache Service → S3
...
```
Your App → Python Client → FastAPI TestClient → Cache Service
...
```
Your App → Python Client → HTTP (localhost) → Cache Service
...
```
┌─────────────────────────────────────────────────────────┐
│              Cache Service API Layer                     │
│  (Strategy handlers, path resolution, validation)        │
└───────────────────────┬─────────────────────────────────┘
                        │
┌───────────────────────▼─────────────────────────────────┐
...
└───────────────────────┬─────────────────────────────────┘
        ...
        │               │               │
┌───────▼────┐  ┌──────▼─────┐  ┌─────▼──────┐
│ S3 Backend │  │ In-Memory  │  │ Disk/SQLite│
│            │  │  Backend   │  │  Backend   │
...
│   │       └── a1b2c3d4-uuid.json
│   │
│   ├── temporal/            # TEMPORAL strategy
│   │   └── 2024/01/20/15/30/
│   │       └── cache_id.json
│   │
│   ├── temporal_latest/     # TEMPORAL_LATEST strategy
│   │   └── latest-pointer → actual-file
│   │
│   ├── temporal_versioned/  # TEMPORAL_VERSIONED
│   │   └── path/to/key/
│   │       ├── 2024-01-20T15:30:00_v1.json
│   │       └── 2024-01-20T16:45:00_v2.json
│   │
│   └── key-based/           # KEY_BASED strategy
...
- ✅ Efficient updates (only store what changed)
- ✅ Deduplication across namespaces (optional)
...
# Same content = same hash → only stored once
...
You've just:
- ✅ Connected to the cache service
- ✅ Stored JSON data
- ✅ Retrieved it back
...
result = client.store().store__json__cache_key(
    ...
    json_field_path="email"  # ✨ Only hash the email field
)
...
**Important Notes:**
- ✅ Use dot notation: `"field"`, `"parent.child"`, `"deep.nested.field"`
- ✅ Can access array indices: `"items.0.name"` (accesses first item)
- ❌ Cannot select multiple fields: No comma-separated lists
- ❌ Cannot use wildcards: Must specify exact path
- ⚠️ Field must exist: ValueError raised if path not found in data
