...
│
├─ sites/
│  └─ example.com/                           ← cache_key="sites/example.com"
│     ├─ site-info.json                      (Optional site-level entry)
│     └─ pages/
│        └─ articles/
│           └─ hello-world/                  ← cache_key="sites/example.com/pages/articles/hello-world"
│              ├─ page-info.json             (Main cache entry - TEMPORAL_LATEST)
│              └─ data/                      (Child data files - HIGH PERFORMANCE)
│                 ├─ transformations/
│                 │  ├─ html/
│                 │  │  └─ latest.txt        (url-to-html result)
│                 │  ├─ text/
│                 │  │  └─ latest.txt        (url-to-text result)
│                 │  └─ ratings/
│                 │     └─ latest.json       (url-to-ratings result)
│                 └─ metadata/
│                    └─ wcf/
│                       └─ latest.json       (WCF call metadata)
```
...
- ✅ Natural S3/storage hierarchy
- ✅ Human-readable paths
- ✅ Direct retrieval by URL (no separate mapping needed)
- ✅ Easy to list all pages for a site
- ✅ Logical grouping for analytics
...
│       ├── __init__.py
│       └── Proxy__Cache__Service.py              # Main cache service
│
├── schemas/
│   └── cache/                                    # NEW - Cache schemas
│       ├── __init__.py
│       ├── Schema__Cache__Config.py              # Cache configuration
│       ├── Schema__Cache__Page_Entry.py          # Page cache entry schema
│       ├── Schema__Cache__Stats.py               # Cache statistics
│       └── Enum__Cache__Transformation_Type.py   # WCF command types
│
├── service/proxy/
│   ├── Proxy__Service.py                         # MODIFY - Add cache_service
│   ├── Proxy__Stats__Service.py                  # MODIFY - Add cache stats
│   └── wcf/
│       └── Proxy__WCF__Service.py                # MODIFY - Integrate caching
│
└── config.py                                     # MODIFY - Add cache config
```
...
def url_to_cache_key(self, target_url: str) -> str:
    ...
    Examples:
        ...
        → sites/subdomain.example.com/pages/docs
    """
...
def store_transformation(self,
    ...
    Storage structure:
        page (cache_id)/
            data/
                transformations/
                    html/latest.txt           ← content stored here
                metadata/
                    wcf/latest.json           ← metadata stored here
    """
...
def test_wcf_command_to_data_key():
    ...
    assert service._wcf_command_to_data_key("url-to-text") == "transformations/text"
...
def test_multiple_transformations():
    ...
    cache_service.store_transformation(url, "url-to-text", "Plain text...", {})

    # Both should be retrievable
    html = cache_service.get_cached_transformation(url, "url-to-html")
    text = cache_service.get_cached_transformation(url, "url-to-text")
...
def get_cached_transformation(self, target_url, wcf_command):
    ...
    if cached:
        print(f"      💾 Cache HIT  {wcf_command:<20} {target_url}")
        self.increment_cache_hit()
        return cached
    else:
        print(f"      ❌ Cache MISS {wcf_command:<20} {target_url}")
...
def store_transformation(self, target_url, wcf_command, content, metadata):
    ...
    print(f"      💾 Cache SAVE {wcf_command:<20} {target_url}")
    return cache_id
```

Example log output:
```
➡️    📥 GET    example.com                   /articles/hello-world
      ❌ Cache MISS url-to-html           https://example.com/articles/hello-world
      🌐 WCF call  url-to-html           (1234ms)
      💾 Cache SAVE url-to-html           https://example.com/articles/hello-world
⬅️    ✅ ___200 example.com                   /articles/hello-world

➡️    📥 GET    example.com                   /articles/hello-world
      💾 Cache HIT  url-to-html           https://example.com/articles/hello-world
⬅️    ✅ ___200 example.com                   /articles/hello-world
```
...
def get_cached_transformation(self, target_url, wcf_command):
    try:
        ...
        # Log but don't crash
        print(f"⚠️  Cache error (non-fatal): {e}")
...
1. ❌ **Cache warming**: Pre-populating cache with common URLs
2. ❌ **TTL expiration**: Time-based cache invalidation
3. ❌ **Cache management endpoints**: Manual invalidation, refresh
4. ❌ **Request coalescing**: Multiple concurrent requests for same URL
5. ❌ **Error caching**: Caching WCF error responses
6. ❌ **Partial responses**: Only caching successful transformations
...
✅ **Simple API**: High-level methods for WCF transformation caching
✅ **Hierarchical Structure**: Sites → Pages → Transformations
✅ **Performance**: Child data files for fast access
✅ **Type Safety**: Type_Safe schemas throughout
✅ **Statistics**: Built-in cache metrics
✅ **Graceful Degradation**: Cache failures don't break WCF calls
✅ **Observable**: Logging and admin UI integration
